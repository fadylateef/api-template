        ...
        ws.onText { ws, text in
            ...
            DispatchQueue.global().async {
                do {
                    ...
                    try shellOut(to: "sshpass -p'fady123' ssh root@185.101.107.142 \"ffmpeg -i /ssd/videos/\(series_id)/\(videoName) -codec: copy -start_number 0 -hls_time 10 -hls_list_size 0 -f hls /ssd/videos/\(series_id)/\(hlsName) && rm /ssd/videos/\(series_id)/\(videoName)\"")
